...
use fileforge_lib::{
  diagnostic::node::{name::DiagnosticNodeName, reference::DiagnosticReference, DiagnosticNode},
  error::{
    render::{
...
use crate::unmanaged::string_table::{
  error::size::StringTableSizeError, DiagnosticReferenceBuilder,
};
  ...
  for StringTableOutOfBounds<'pool, DIAGNOSTIC_NODE_NAME_SIZE>
{
  fn with_report<Cb: FnMut(Report<DIAGNOSTIC_NODE_NAME_SIZE>) -> ()>(&self, mut callback: Cb) {
    ...
    let string_table_size_dr = self
      .string_table_size_dr
      .as_ref()
      .map(|v| v.relocate_build(string_table_dr))
      .flatten();
    ...
    let primary_note_width = self
      .string_table_size
      .map(|size| FormattedUnsigned::new(size as u64));
    ...
    let size_note_size = self
      .string_table_size
      .map(|size| FormattedUnsigned::new(size as u64));
    let size_note_size_raw =
      &string_table_size_dr.map(|(size, _)| FormattedUnsigned::new(size as u64));
    ...
    let position_note_position = FormattedUnsigned::new(self.string_table_position as u64)
      .with_base(16)
      .with_prefix("0x");
    ...
    if !self.string_table_parent.family_exists()
      || !string_table_size_dr
        .map(|dr| dr.1.family_exists())
        .unwrap_or(true)
      || !self.string_table_position_dr.family_exists()
    {
        ...
        report = report
          .with_note(|| {
            ReportNote::new(size_note)
              .with_location(size_dr.1, size_note_size_raw.as_ref().unwrap())
              .unwrap()
          })
          .unwrap();
      }
      ...
      report = report
        .with_note(|| {
          ReportNote::new(position_note)
            .with_location(self.string_table_position_dr, &position_note_position)
            .unwrap()
        })
        .unwrap()
    }
      ...
      report = report
        .with_note(|| {
          ReportNote::new(primary_note)
            .with_unvalued_location(string_table_dr.parent_reference().unwrap())
            .unwrap()
        })
        .unwrap();
    } else {
