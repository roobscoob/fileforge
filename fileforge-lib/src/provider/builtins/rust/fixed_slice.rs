...
pub struct RustFixedSliceProvider<'l, T, const SIZE: usize> {
  data: &'l [T; SIZE],
}

impl<'l, T, const SIZE: usize> From<&'l [T; SIZE]> for RustFixedSliceProvider<'l, T, SIZE> {
  fn from(data: &'l [T; SIZE]) -> Self {
    Self { data }
  }
}

impl<'l, T, const SIZE: usize> Provider for RustFixedSliceProvider<'l, T, SIZE> {
  type Type = T;
  ...
  async fn read<const READ_SIZE: usize, V>(&self, offset: u64, _hint: ReadHint, reader: impl AsyncFnOnce(&[T; READ_SIZE]) -> V) -> Result<V, ProviderReadError<Self::ReadError>> {
