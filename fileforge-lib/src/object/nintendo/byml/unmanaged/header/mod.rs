...
use crate::{diagnostic::node::{name::DiagnosticNodeName, reference::DiagnosticReference}, provider::r#trait::Provider, reader::{self, error::parse::ParseError, r#trait::readable::FixedSizeReadable, Reader}, object::endianness::EndiannessMarker};
...
impl<'pool, const DIAGNOSTIC_NODE_NAME_SIZE: usize> BymlHeader<'pool, DIAGNOSTIC_NODE_NAME_SIZE> {
  ...
  pub fn endianness_diagnostic(&self) -> DiagnosticReference<'pool, DIAGNOSTIC_NODE_NAME_SIZE> {
    self.diagnostic_reference.create_physical_child(0, 2, DiagnosticNodeName::from("Endianness"))
  }
  ...
  pub fn key_table_offset_diagnostic(&self) -> DiagnosticReference<'pool, DIAGNOSTIC_NODE_NAME_SIZE> {
    self.diagnostic_reference.create_physical_child(4, 4, DiagnosticNodeName::from("Key Table Offset"))
  }

  pub fn string_table_offset_diagnostic(&self) -> DiagnosticReference<'pool, DIAGNOSTIC_NODE_NAME_SIZE> {
    self.diagnostic_reference.create_physical_child(8, 4, DiagnosticNodeName::from("String Table Offset"))
  }

  pub fn root_data_offset_diagnostic(&self) -> DiagnosticReference<'pool, DIAGNOSTIC_NODE_NAME_SIZE> {
    self.diagnostic_reference.create_physical_child(12, 4, DiagnosticNodeName::from("Root Node Offset"))
  }
}

impl<'pool, const DIAGNOSTIC_NODE_NAME_SIZE: usize> FixedSizeReadable<'pool, DIAGNOSTIC_NODE_NAME_SIZE, 0x10> for BymlHeader<'pool, DIAGNOSTIC_NODE_NAME_SIZE> {
  type Argument = ();
