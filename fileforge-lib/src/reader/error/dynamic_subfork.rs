...
pub enum DynamicSubforkError<'l, 'pool, const NODE_NAME_SIZE: usize, S: DynamicPartitionableStream<'l, NODE_NAME_SIZE>> {
  Stream(S::PartitionError),
  OutOfBounds(SeekOutOfBounds<'pool, NODE_NAME_SIZE>),
}

impl<'l, 'pool, const NODE_NAME_SIZE: usize, S: DynamicPartitionableStream<'l, NODE_NAME_SIZE>> FileforgeError<'pool, NODE_NAME_SIZE> for DynamicSubforkError<'l, 'pool, NODE_NAME_SIZE, S> {
