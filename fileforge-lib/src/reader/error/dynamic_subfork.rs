use crate::{diagnostic::pool::DiagnosticPoolProvider, error::{report::Report, FileforgeError}, stream::DynamicPartitionableStream};
...
impl<'l, 'pool, S: DynamicPartitionableStream<'l>> FileforgeError for DynamicSubforkError<'l, 'pool, S> {
  fn render_into_report<'pool_ref, const ITEM_NAME_SIZE: usize, P: DiagnosticPoolProvider>(&self, provider: &'pool_ref P, callback: impl for<'tag, 'b, 'p2> FnMut(Report<'tag, 'b, 'p2, 'pool_ref, ITEM_NAME_SIZE, P>) -> ()) {
    match self {
