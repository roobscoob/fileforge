...
impl<'pool, Re: ProviderError, const DIAGNOSTIC_NODE_NAME_SIZE: usize>
  ParsePrimitiveError<'pool, Re, DIAGNOSTIC_NODE_NAME_SIZE>
{
...
impl<'pool, Re: ProviderError, const DIAGNOSTIC_NODE_NAME_SIZE: usize>
  ...
  for ParsePrimitiveError<'pool, Re, DIAGNOSTIC_NODE_NAME_SIZE>
{
...
>
{
  ...
  >(
    self,
    cb: Cb,
  ) -> Result<Result<Success, UnderlyingProviderReadError<'pool, Re, DIAGNOSTIC_NODE_NAME_SIZE>>, Ne>;
}

impl<'pool, Success, Re: ProviderError, const DIAGNOSTIC_NODE_NAME_SIZE: usize>
  ParsePrimitiveErrorResultExtension<'pool, Success, Re, DIAGNOSTIC_NODE_NAME_SIZE>
  for Result<Success, ParsePrimitiveError<'pool, Re, DIAGNOSTIC_NODE_NAME_SIZE>>
{
  fn map_out_of_bounds<
    Ne,
    Cb: FnOnce(ReadOutOfBoundsError<'pool, DIAGNOSTIC_NODE_NAME_SIZE>) -> Ne,
