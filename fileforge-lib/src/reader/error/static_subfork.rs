...
impl<'l, 'pool, const SIZE: usize, S: StaticPartitionableStream<'l, SIZE>> FileforgeError for StaticSubforkError<'l, 'pool, SIZE, S> {
  fn render_into_report<'pool_ref, const ITEM_NAME_SIZE: usize, P: DiagnosticPoolProvider>(&self, provider: &'pool_ref P, callback: impl for<'tag, 'b, 'p2> FnMut(crate::error::report::Report<'tag, 'b, 'p2, 'pool_ref, ITEM_NAME_SIZE, P>) -> ()) {
    match self {
