...
pub struct ReadOutOfBoundsError<'pool_lifetime, const DIAGNOSTIC_NODE_NAME_SIZE: usize> {
  pub read_offset: u64,
  pub read_size: u64,
  pub reader_size: u64,
  pub reader_diagnostic: DiagnosticReference<'pool_lifetime, DIAGNOSTIC_NODE_NAME_SIZE>,
}

impl<'pool_lifetime, const DIAGNOSTIC_NODE_NAME_SIZE: usize> ReadOutOfBoundsError<'pool_lifetime, DIAGNOSTIC_NODE_NAME_SIZE> {
  pub fn from_slice_out_of_bounds_error(error: SliceOutOfBoundsError, diagnostic: DiagnosticReference<'pool_lifetime, DIAGNOSTIC_NODE_NAME_SIZE>) -> ReadOutOfBoundsError<'pool_lifetime, DIAGNOSTIC_NODE_NAME_SIZE> {
...
impl<'pool_lifetime, const DIAGNOSTIC_NODE_NAME_SIZE: usize> Error<DIAGNOSTIC_NODE_NAME_SIZE> for ReadOutOfBoundsError<'pool_lifetime, DIAGNOSTIC_NODE_NAME_SIZE> {
  fn with_report<Cb: FnMut(Report<DIAGNOSTIC_NODE_NAME_SIZE>) -> ()>(&self, mut callback: Cb) {
    if !self.reader_diagnostic.family_exists() {
      ...
      return callback(
        Report::new::<ReadOutOfBoundsError<'pool_lifetime, DIAGNOSTIC_NODE_NAME_SIZE>>(ReportKind::Error, "Read Out Of Bounds")
