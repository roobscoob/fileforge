...
impl<
    ...
    Re: ProviderError,
    const DIAGNOSTIC_NODE_NAME_SIZE: usize,
  > ParseErrorResultExtension<'pool, Success, T, Re, DIAGNOSTIC_NODE_NAME_SIZE>
  for Result<Success, ParseError<'pool, T, Re, DIAGNOSTIC_NODE_NAME_SIZE>>
{
  fn map_domain_specific<Ne, Cb: FnOnce(T) -> Ne>(
    self,
    cb: Cb,
...
impl<
    ...
    Re: ProviderError,
    const DIAGNOSTIC_NODE_NAME_SIZE: usize,
  > ParseError<'pool, T, Re, DIAGNOSTIC_NODE_NAME_SIZE>
{
  pub fn domain_err(value: T) -> Self { ParseError::DomainSpecific(value) }
  ...
  pub fn map_domains<N: Error<DIAGNOSTIC_NODE_NAME_SIZE>, M: FnOnce(T) -> N>(
    self,
    mapper: M,
  ) -> ParseError<'pool, N, Re, DIAGNOSTIC_NODE_NAME_SIZE> {
    match self {
      Self::DomainSpecific(t) => {
        ParseError::<'pool, N, Re, DIAGNOSTIC_NODE_NAME_SIZE>::domain_err(mapper(t))
      }
      Self::OutOfBounds(e) => ParseError::<'pool, N, Re, DIAGNOSTIC_NODE_NAME_SIZE>::OutOfBounds(e),
      Self::UnderlyingProviderReadError(re) => {
        ParseError::<'pool, N, Re, DIAGNOSTIC_NODE_NAME_SIZE>::UnderlyingProviderReadError(re)
      }
    }
  }
}
...
impl<
  ...
  for ParseError<'pool, T, Re, DIAGNOSTIC_NODE_NAME_SIZE>
{
...
impl<
    ...
    Re: ProviderError,
    const DIAGNOSTIC_NODE_NAME_SIZE: usize,
  > From<DomainError<T>> for ParseError<'pool, T, Re, DIAGNOSTIC_NODE_NAME_SIZE>
{
...
impl<
    ...
    Re: ProviderError,
    const DIAGNOSTIC_NODE_NAME_SIZE: usize,
  > Error<DIAGNOSTIC_NODE_NAME_SIZE> for ParseError<'pool, T, Re, DIAGNOSTIC_NODE_NAME_SIZE>
{
