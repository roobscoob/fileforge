...
impl<const NODE_NAME_SIZE: usize, UserSeek: UserSeekError<NODE_NAME_SIZE>> From<StreamSeekOutOfBoundsError> for StreamSeekError<NODE_NAME_SIZE, UserSeek> {
  fn from(value: StreamSeekOutOfBoundsError) -> Self { Self::OutOfBounds(value) }
}

impl<const NODE_NAME_SIZE: usize, UserSeek: UserSeekError<NODE_NAME_SIZE>> From<UserSeek> for StreamSeekError<NODE_NAME_SIZE, UserSeek> {
