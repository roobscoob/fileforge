...
use self::{canvas::RenderBufferCanvas, cell::{tag::context::{CellTagContext, RenderMode}, RenderBufferCell}};
...
impl<'buffer, 'tag> RenderBuffer<'buffer, 'tag> {
  ...
  pub fn can_set_char(&self, position: RenderPosition, c: Grapheme) -> bool {
    if (position.column() + c.width()) > self.line_width { return false }
  ...
  pub fn set_char(&mut self, position: RenderPosition, c: RenderBufferCell<'tag>) -> bool {
    ...
    if position.line() >= (self.height + self.line_offset) { return true }
    if position.line() < self.line_offset { return true }
    ...
    for line in self.buffer.chunks_exact(self.line_width) {
      ...
      for (cell, index) in line.iter().zip(0..) {
        ...
        if let Some(cell_tag) = my_cell_tag {
          cell_tag.render_into(into, *cell.contents(), CellTagContext {
            mode,
