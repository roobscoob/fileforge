...
pub trait Readable<'pool, 'reader, S: ReadableStream<Type = u8>>: Sized + 'reader {
  type Error: FileforgeError;
  type Argument;

  async fn read(reader: &'reader mut BinaryReader<'pool, S>, argument: Self::Argument) -> Result<Self, Self::Error>;
}
