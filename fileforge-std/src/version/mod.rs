...
impl<const SEGMENTS: usize> Version<SEGMENTS> {
  pub fn new_raw(contents: [u8; SEGMENTS]) -> Self { Self { versions: contents } }
}

impl Version<1> {
  pub fn new(version: u8) -> Self { Self { versions: [version] } }
}

impl Version<2> {
  pub fn new(major: u8, minor: u8) -> Self { Self { versions: [major, minor] } }
}

impl Version<3> {
  pub fn new(major: u8, minor: u8, patch: u8) -> Self { Self { versions: [major, minor, patch] } }
}

impl Version<4> {
  pub fn new(major: u8, minor: u8, patch: u8, revision: u8) -> Self { Self { versions: [major, minor, patch, revision] } }
}
