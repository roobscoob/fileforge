...
impl<'pool, const DIAGNOSTIC_NODE_NAME_SIZE: usize, const MAGIC_SIZE: usize>
  MagicError<'pool, DIAGNOSTIC_NODE_NAME_SIZE, MAGIC_SIZE>
{
  fn print_with_report<Cb: FnMut(Report<DIAGNOSTIC_NODE_NAME_SIZE>) -> ()>(
    ...
    let magic_info = Text::new().push(
      "A broken magic indicates the slice is entirely invalid or corrupt.",
    ...
    callback(
      Report::new::<MagicError<'pool, DIAGNOSTIC_NODE_NAME_SIZE, MAGIC_SIZE>>(
        ReportKind::Error,
        "Invalid Magic",
      )
      .with_info_line(&info)
...
impl<'pool, const DIAGNOSTIC_NODE_NAME_SIZE: usize, const MAGIC_SIZE: usize>
  Error<DIAGNOSTIC_NODE_NAME_SIZE> for MagicError<'pool, DIAGNOSTIC_NODE_NAME_SIZE, MAGIC_SIZE>
{
