...
impl<'pool> FileforgeError for ReaderExhaustedError<'pool, true> {
  fn render_into_report<'pool_ref, const ITEM_NAME_SIZE: usize, P: DiagnosticPoolProvider>(
    &self,
    provider: &'pool_ref P,
    mut callback: impl for<'a, 'b, 'p2> FnMut(crate::error::report::Report<'a, 'b, 'p2, 'pool_ref, ITEM_NAME_SIZE, P>) -> (),
  ) {
    let context = ErrorContext::new(provider)
      .with("length", self.length.reference())
      .with("stream_length", self.stream_length.reference())
      .with("container", self.container);
    ...
    if let Some(location) = context.get("container") {
      report
        .add_note(
          ReportNote::new(const_text!([&REPORT_INFO_LINE_TEXT] "This is the container"))
            .with_tag(&REPORT_INFO_LINE_TEXT)
            .with_unvalued_location(location)
            .unwrap(),
        )
        .unwrap();
    }
    ...
    if let Some(location) = context.get("length") {
      report
        .add_note(
          ReportNote::new(&read_text)
            .with_tag(&REPORT_INFO_LINE_TEXT)
            .with_raw_location(ReportLocation {
              value: Some(&length_base_16),
              reference: location,
            })
            .unwrap(),
        )
