...
impl Yaz0State {
  ...
  #[inline]
  fn readback(&self) -> ReadbackReference {
    self.seekback_buffer.as_slices().into()
  }

  #[inline]
  fn last_n(&self, n: usize) -> Option<ReadbackReference<'_>> {
  ...
  #[inline]
  pub fn take(&mut self, desired: usize) -> ReadbackReference<'_> {
    let ub = self.unread_bytes as usize;
    let taken = desired.min(ub);
    self.unread_bytes -= taken as u16;
  ...
  #[inline]
  pub fn feed(&mut self, operation: Operation) -> Result<(), MalformedStream> {
    if (self.unread_bytes + operation.len()) as usize > SEEKBACK_BUFFER_LENGTH {
      panic!("Seekback Buffer Overflow")
    }
    ...
    match operation {
      ...
      Operation::Readback { offset, length } => {
        for _ in 0..length.get() {
          self.push_byte(
            self
              .last_n(offset as usize)
              .ok_or(MalformedStream::SeekbackOutOfBounds {
                seekback_offset: offset,
                seekback_size: self.seekback_buffer.len() as u16,
              })?
              .get(0)
              .ok_or(MalformedStream::EmptySeekback)?,
          )
        }

        self.unread_bytes += length.get();
      }
    }
...
mod tests {
  ...
  // Helper: convert a ReadbackReference into a Vec<u8> for easy equality checks.
  fn rb_to_vec(r: ReadbackReference<'_>) -> Vec<u8> {
  ...
  fn empty_is_quiet() {
    ...
    let err = st.feed(Operation::Readback { offset: 1, length: nz(1) }).unwrap_err();
    match err {
  ...
  fn literals_accumulate_then_take_in_oldest_first_order() {
    ...
    for &b in b"abcd" {
      st.feed(Operation::Literal(b)).unwrap();
    }
  ...
  fn take_truncates_to_available() {
    ...
    st.feed(Operation::Literal(b'X')).unwrap(); // unread = 1
    let t = st.take(5);
  ...
  fn readback_simple_copy_sliding_source() {
    ...
    st.feed(Operation::Readback { offset: 3, length: nz(3) }).unwrap();
  ...
  fn readback_chained_copies_from_growing_window() {
    ...
    st.feed(Operation::Readback { offset: 2, length: nz(2) }).unwrap();
    st.feed(Operation::Readback { offset: 2, length: nz(2) }).unwrap();
  ...
  fn readback_fails_when_offset_exceeds_history() {
    ...
    let err = st.feed(Operation::Readback { offset: 3, length: nz(1) }).unwrap_err();
  ...
  fn overflow_guard_panics_on_readback_beyond_capacity() {
    ...
    // Seed minimally.
    st.feed(Operation::Literal(b'X')).unwrap();

    // Fill to capacity exactly: remaining = capacity - 1
    let remaining = (SEEKBACK_BUFFER_LENGTH - 1) as u16;
    st.feed(Operation::Readback { offset: 1, length: nz(remaining) }).unwrap();

    // One more byte via readback would exceed -> panic.
    st.feed(Operation::Readback { offset: 1, length: nz(1) }).unwrap();
  }
  ...
  fn ring_buffer_eviction_without_unread_overflow() {
    ...
    for i in 0..total {
      st.feed(Operation::Literal((i % 251) as u8)).unwrap();
    ...
    st.feed(Operation::Readback { offset: 5, length: nz(5) }).unwrap();
  ...
  fn mixed_sequence_behaves_as_queue_of_unread() {
    ...
    // unread currently "c"
    st.feed(Operation::Readback { offset: 1, length: nz(2) }).unwrap();
  ...
  fn readback_offset_one_is_repeat_last_byte() {
    ...
    st.feed(Operation::Readback { offset: 1, length: nz(5) }).unwrap();
  ...
  fn readback_exact_window_boundary() {
    ...
    st.feed(Operation::Readback { offset: 4, length: nz(4) }).unwrap();
